...
class DailyActivity(Base):
    ...
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False, index=True)
    client_id = Column(Integer, ForeignKey("client_accounts.id"), nullable=True, index=True)
    source_client_task_id = Column(Integer, ForeignKey("client_tasks.id"), nullable=True, index=True)
    post_group_id = Column(String(40), nullable=True, index=True)
    continued_from_activity_id = Column(Integer, ForeignKey("daily_activities.id"), nullable=True, index=True)
    continued_to_activity_id = Column(Integer, ForeignKey("daily_activities.id"), nullable=True, unique=True, index=True)
...
class CashRequisitionRequest(Base):
    __tablename__ = "cash_requisition_requests"
...
class AuthorityToIncurRequest(Base):
    __tablename__ = "authority_to_incur_requests"
    ...
    incurred_note = Column(Text, nullable=True)
    incurred_by_id = Column(Integer, ForeignKey("users.id"), nullable=True, index=True)
